...
def register(request):
    ...
    return render(request, 'prodapp/register.html', {'u_form': u_form})
...
def profile(request):
    ...
    return render(request, 'prodapp/profile.html', context)
...
def submit_todo(request):
    ...
    progress, created = Progress.objects.update_or_create(user=request.user,
                                                          date=request.POST['date'], defaults={'final_score': day_score})
    progress.save()
